...
在对任何请求进行签名之前，您必须通过 bcce 网站【用户中心】-【API】创建一个API key。 创建key后，您将获得3个必须记住的信息：
* API Key
...
```bash
curl "https://www.bcce.com/api/v1/orders?limit=100"       
```
* 获取获取深度信息，以 LTC-BTC 币对为例
```java
Timestamp = 1540286290170 
Method = "GET"
requestPath = "/api/v1/public/products/LTC-BTC/orderbook"
...
```
Message = '1540286290170GET/api/v1/public/products/LTC-BTC/orderbook?size=100'  
```
* 下单，以 LTC-BTC 币对为例
...
REST访问的根URL：`https://www.bcce.com`
...
```http
    # Request
    GET /api/v1/public/products/<code>/orderbook
```
```javascript
    # Response
    {
...
```http
    # Request
    GET /api/v1/public/products/<code>/ticker
```
...
```http
    # Request
    GET /api/v1/public/products/<code>/fills
```
```javascript
    # Response
    [
...
```http
    # Request
    GET  /api/v1/public/products/<code>/candles?type=1min&start=start_time&end=end_time
```
...
|---|----|----|----|
|code|String|是|币对,如 btc_usdt|
|orderId|String|是|需要撤销的未成交委托的id（注：url 中的 orderId，如 ：/api/v1/orders/10010）
...
```http   
    # Request
    GET /api/v1/orders?code=eth_btc&status=open
```
```javascript
    # Response
    {
...
[bcce]: https://www.bcce.com 
[English Docs]: https://github.com/bcce/bcce-official-api-docs/blob/master/README_EN.md
